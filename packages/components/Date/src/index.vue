<template>
  <div v-click-outside :class="['y-date-box', 'y-date-default']">
    <div class="y-date-inner-box">
      <y-input
        v-model="dateVal"
        :size="size"
        :clearable="clearable"
        ...
        :disabled="disabled"
        :focus-color="customActiveColor"
        :form="form"
        @clear="clear"
      />
      <transition name="slide-fade">
        <div v-if="show" :class="['y-date-option', `y-date-option-${size}`, customClass]">
          <div class="y-date-fipx-box">
            <!-- 年月日 -->
            <div class="y-date-date-box">
              <div :class="['y-date-header', `y-date-header-${size}`]">
                <div class="y-date-btn-left">
                  <span v-if="monthShow"
                    ><i class="m-icon-arrow-double-left" @click="onlyYearPrev"
                  /></span>
                  <span v-if="!dateShow && !monthShow"
                    ><i class="m-icon-arrow-double-left" @click="getPrevYear"
                  /></span>
                  <span v-if="dateShow"
                ...
                <div class="y-date-btn-right">
                  <span v-if="dateShow && type == 'date'"
                    ><i class="m-icon-arrow-right" @click="nextMonth"
                  /></span>
                  <span v-if="dateShow"
                    ><i class="m-icon-arrow-double-right" @click="nextYear"
                  /></span>
                  <span v-if="!dateShow && !monthShow"
                    ><i class="m-icon-arrow-double-right" @click="getNextYear"
                  /></span>
                  <span v-if="monthShow"
                    ><i class="m-icon-arrow-double-right" @click="onlyYearNext"
                  /></span>
                </div>
              </div>
              <div
                v-show="dateShow && type == 'date'"
                :class="['y-date-content-box', `y-date-content-box-${size}`]"
              >
                <div class="y-date-year-month-box">
                  <table>
                    <tbody>
                      <tr>
                        <th><span>一</span></th>
...
import yInput from '../../Input/src/index.vue';
  ...
  beforeMount(el) {
    const handler = (e) => {
      if (!props.disabled) {
        ...
        } else if (!e.target.className.includes('date-span-disabled')) {
...
const getYearList = (startYear) => {
  ...
  for (let i = 0; i < 10; i++) {
    list.push(
      new Date(selDate.value.year) - Number.parseInt(new Date(selDate.value.year) % 10) + i,
    );
  }
  return list;
};
...
.y-date-default {
  ...
  .y-date-content-box,
  .y-date-content-box-default {
    width: 100%;
    height: auto;
    overflow: hidden;
    padding: 0 10px;
    box-sizing: border-box;
    .y-date-year-month-box {
      width: 100%;
      height: auto;
      overflow: hidden;
      padding-bottom: 10px;
